...
def parse_input_file(filename):
    ...
    while line:
        ...
        if ("component_id:" in line) and ("sub_component" not in line):
...
for sensor in all_sensors:
    sensor_bak = ''.join(sensor.split(":")[0]) + '_' + ''.join(sensor.split(":")[-1:])
    filename = current_time + "_" + sensor_bak + ".csv"
    res = open(filename, 'w')
...
for sensor in all_sensors:
    ...
    sensor_bak = ''.join(sensor.split(":")[0]) + '_' + ''.join(sensor.split(":")[-1:])
    filename = current_time + "_" + sensor_bak + ".csv"
    cmd = "cat " + filename + " | grep -v iLatency | awk -F \",\" '{ print $NF }'"
    s = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE).stdout
    all_ilatency = s.read().splitlines()
    for i in range(len(all_ilatency)):
        all_ilatency[i]=int(all_ilatency[i])
    
    max_latency = max(all_ilatency)
    min_latency = min(all_ilatency)
    average_latency = sum(all_ilatency)/len(all_ilatency)    
    
    p50 = percentile(all_ilatency, 50)
    p60 = percentile(all_ilatency, 60)
    p70 = percentile(all_ilatency, 70)
    p80 = percentile(all_ilatency, 80)
    p85 = percentile(all_ilatency, 85)
    p90 = percentile(all_ilatency, 90)
    p95 = percentile(all_ilatency, 95)
    p100 = percentile(all_ilatency, 100)
    ...
    temp = "Sensor,Seq_no,Producer_TS,Grpc_TS,RE_creation_ts,Wrap_No,Inter-Packet-Delay,Is_Wrap,Producer_wrap_skew,iLatency,Time/wrap,Pkt/wrap\n"
    res.write(temp)
    ...
    for line in open(filename):
        if "sensor_" in line:
            ...
            if seq == 0 and is_1st_seq:
                ...
                temp = "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (sensor,seq,pro_ts, grpc_ts, re_ts, wrap, "N/A", "Y", "N/A", ilatency, "N/A", "N/A")
                res.write(temp)
                is_1st_seq = False
            else:
                '''
                ...
                if re_ts != previous_re_ts:
                    ...
                    temp= "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (sensor,seq,pro_ts, grpc_ts, re_ts, wrap, packetdelay, "Y", producer_skew, ilatency, time_per_wrap, pkt_per_wrap)
                    res.write(temp)
                    previous_wrap_seq = seq
                    previous_wrap_ts = pro_ts
                    
                else:
                    temp = "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (sensor,seq,pro_ts, grpc_ts, re_ts, wrap, packetdelay, "N/A", "N/A", ilatency, "N/A", "N/A")
                    res.write(temp)
                previous_grpc_ts = grpc_ts
                previous_producer_ts = pro_ts
                previous_re_ts = re_ts
