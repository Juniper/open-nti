                    ...
                    ## we use single-value to deal with RE output
                        ...
                        # regex has to be: \s*([0-9]+)\s+\w+\s+([0-9]+)\s+\S+\s+\d*\S*\s+(\d*\S*)\s+(\d+\S*)\s+\S+\s+\S+\s+\S+\s+(\S+)\s+EvoAftManBt-mai{EvoAftManBt-mai}$
                            ...
                            if text_matches:
                                ...
                                logger.error('[%s]: no matches found on regex especified on parser: %s', host, regex_command)
                        else:
...
hosts_yaml_file = BASE_DIR_INPUT + default_variables['test_hosts_file']
